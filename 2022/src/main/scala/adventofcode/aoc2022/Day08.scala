	...
	val trees: Grid[Tree] =
		Grid.unit:
			input
				...
				.zipWithIndex
				.map:
					(l, y) => l.zipWithIndex.map:
						(c, x) => Tree(Pos(x, y), c.asDigit)
				.toVector

	case class Tree(pos: Pos, height: Int):
		def isVisible(trees: Grid[Tree]): Boolean =
			val (left, right) = trees.getRow(pos.y).filterNot(_.pos == pos).splitAt(pos.x)
			val (up, down)    = trees.getCol(pos.x).filterNot(_.pos == pos).splitAt(pos.y)
		...
		def scenicScore(trees: Grid[Tree]): Int =
			...
			(left.length  min (left.reverse.takeWhile(_.height < height).length + 1)) *
