...
case class Pos4D(x: Int, y: Int, z: Int, w: Int):
	...
	def manhattan(that: Pos4D): Long =
		math.abs(math.max(x, that.x) - math.min(x, that.x)) +
			...
			math.abs(math.max(w, that.w) - math.min(w, that.w))

	def axisOffsets: Set[Pos4D] =
		Set(
			Pos4D(x + 1, y, z, w), Pos4D(x - 1, y, z, w),
			Pos4D(x, y + 1, z, w), Pos4D(x, y - 1, z, w),
			Pos4D(x, y, z - 1, w), Pos4D(x, y, z + 1, w),
			Pos4D(x, y, z, w - 1), Pos4D(x, y, z, w + 1),
		)
