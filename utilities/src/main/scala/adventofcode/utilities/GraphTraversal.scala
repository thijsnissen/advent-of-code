...
object GraphTraversal:
	...
	private def updateToVisit[A](as: SortedSet[A], toVisit: Vector[(A, A)], visited: Vector[(A, A)], from: A): Vector[(A, A)] =
		as
			.view
	...
	extension[A] (self: Graph[A])
		def breadthFirstSearch(source: A)(target: A => Boolean): Option[Vector[(A, A)]] =
			if target(source) then
				Some(Vector(source -> source))
			else
				loop(
					Vector.empty[(A, A)] ++ self.run(source).toVector.map(_ -> source), Vector(source -> source), false)(target)

		def breadthFirstSearchPathTo(source: A)(target: A => Boolean): Option[List[A]] =
		...
		def depthFirstSearch(source: A)(target: A => Boolean): Option[Vector[(A, A)]] =
			if target(source) then
				Some(Vector(source -> source))
			else
				loop(Vector.empty[(A, A)] ++ self.run(source).toVector.map(_ -> source), Vector(source -> source), true)(target)

		@annotation.tailrec
		private def loop(toVisit: Vector[(A, A)], visited: Vector[(A, A)], lifo: Boolean)(target: A => Boolean): Option[Vector[(A, A)]] =
			toVisit match
