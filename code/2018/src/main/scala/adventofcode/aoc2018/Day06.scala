...
object Day06 extends AdventOfCode(Prod):
  ...
  case class Coordinate(id: Int, x: Int, y: Int):
    def distanceTo(tx: Int, ty: Int): Int = (x - tx).abs + (y - ty).abs

  lazy val maxX: Int = myInput.map(_.x).max + 1
  lazy val maxY: Int = myInput.map(_.y).max + 1

  lazy val grid1: Vector[Vector[Int]] = (0 until maxX).foldLeft(
    Vector.fill(maxY)(Vector.fill(maxX)(-1))
  ): (state, x) =>
  ...
  lazy val grid2: Vector[Vector[Int]] = (0 until maxX).foldLeft(
    Vector.fill(maxY)(Vector.fill(maxX)(-1))
  ): (state, x) =>
    (0 until maxY).foldLeft(state): (state, y) =>
