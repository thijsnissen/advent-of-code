...
object Day22 extends AdventOfCode:
	...
	object Regions:
		def unit(mouthOfCave: Pos, target: Pos, depth: Int, multiplier: Int): Regions =
			val init: Regions =
			...
			val regions: Map[Pos, RegionType] =
				Box(mouthOfCave, target * multiplier)
					...
					.foldLeft(Map.empty[Pos, Int]):
						(acc, p) => getErosionLevels(p, init, acc)
					.map((p, el) => p -> RegionType.fromErosionLevel(el))

			init.copy(regions = regions)

		def getErosionLevels(pos: Pos, regions: Regions, acc: Map[Pos, Int]): Map[Pos, Int] =
			def erosionLevel(geologicIndex: Int): Int =
				(geologicIndex + regions.depth) % 20183

			pos match
				case p if p == regions.mouthOfCave || p == regions.target => acc + (pos -> erosionLevel(0))
				...
				case Pos(x, y) =>
					...
					(levelsA ++ levelsB) + (pos -> erosionLevel(levelsA(Pos(x - 1, y)) * levelsB(Pos(x, y - 1))))
