		...
		def fromString(s: String): Sample =
			s match
				case sampleRegex(b0, b1, b2, b3, i0, i1, i2, i3, a0, a1, a2, a3) =>
					Sample(
						Instruction(i0.toInt, i1.toInt, i2.toInt, i3.toInt),
						Device.unit(Map(0 -> b0.toInt, 1 -> b1.toInt, 2 -> b2.toInt, 3 -> b3.toInt)),
						Device.unit(Map(0 -> a0.toInt, 1 -> a1.toInt, 2 -> a2.toInt, 3 -> a3.toInt))
					)
	...
	object Device:
		...
		def analyzeSamplesBySample: List[List[String]] =
			samples.foldLeft(List.empty[List[String]]):
				(acc, sample) =>
					val result = unorderedOpcodes.map:
						opcode =>
							if sample.before.run(opcode, sample.inst.a, sample.inst.b, sample.inst.c) == sample.after then
								opcode
							else
								""
		...
		@annotation.tailrec
		def orderedOpcodes(opcodes: List[(String, List[Int])], acc: List[(Int, String)]): List[String] =
